...
__length = getline(&_INPUT__, &iptl, stdin);
if (__length == -1)
{
if (_MODE_)
write(STDOUT_FILENO, "\n", 1);
...
_cmdsCount_++;
if (__builtIn__(&_PATHDIR_, &_args__, &argv, &_path__, &_INPUT__, &
_str_cc_, &_cmdsCount_)
|| __handleInvalidCMD__(&_PATHDIR_, &_args__, &argv, &_path__, &_str_cc_, &
_INPUT__, &PATH_, &isValidCMD, &_cmdsCount_))
continue;
__startForkin__(&_pid, &_PATHDIR_, &_args__, &env,
		&_path__, &_INPUT__, &_status, &_ch_exit_status);
}
return (0);
}
